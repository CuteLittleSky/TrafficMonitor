...
{
    for (auto* taskbar_wnd : m_tBarDlg_all)
    {
    ...
    m_tBarDlg_all.clear();
    m_tBarDlg = nullptr;
    m_taskbar_menu_source_hwnd = nullptr;
...
void CTrafficMonitorDlg::OpenTaskBarWnd()
{
    ...
    std::vector<HWND> taskbar_handles;
    HWND h_primary_taskbar = ::FindWindow(_T("Shell_TrayWnd"), NULL);
    if (h_primary_taskbar != nullptr)
        taskbar_handles.push_back(h_primary_taskbar);

    if (CWindowsSettingHelper::IsTaskbarShowingInAllDisplays())
    {
        ...
        {
            taskbar_handles.insert(taskbar_handles.end(), secondary_taskbars.begin(), secondary_taskbars.end());
        }
        else if (theApp.m_taskbar_data.show_taskbar_wnd_in_secondary_display && !secondary_taskbars.empty())
        {
            ...
            taskbar_handles.clear();
            taskbar_handles.push_back(secondary_taskbars[index]);
        }
    }

    for (size_t i = 0; i < taskbar_handles.size(); ++i)
    {
        CTaskBarDlg* taskbar_wnd{};
        if (theApp.m_win_version.IsWine())
            taskbar_wnd = new CWineTaskbarDlg();
        else if (theApp.IsWindows11Taskbar())
            taskbar_wnd = new CWin11TaskbarDlg();
        else
            taskbar_wnd = new CClassicalTaskbarDlg();

        TCHAR class_name[128]{};
        ::GetClassName(taskbar_handles[i], class_name, _countof(class_name));
        bool is_secondary_taskbar = (_tcscmp(class_name, _T("Shell_SecondaryTrayWnd")) == 0);
        taskbar_wnd->SetTaskbarTarget(taskbar_handles[i], is_secondary_taskbar);
        ...
        case RenderType::D2D1_WITH_DCOMPOSITION:
            taskbar_wnd->Create(IDD_TASK_BAR_DIALOG_NOREDIRECTIONBITMAP, this);
            break;
        // 包括RenderType::D2D1在内的其他值
        default:
            taskbar_wnd->Create(IDD_TASK_BAR_DIALOG, this);
            break;
        }
        taskbar_wnd->ShowWindow(SW_SHOW);
        m_tBarDlg_all.push_back(taskbar_wnd);
    }

    if (!m_tBarDlg_all.empty())
        m_tBarDlg = m_tBarDlg_all.front();
}

CTaskBarDlg* CTrafficMonitorDlg::GetTaskbarWndByHwnd(HWND hwnd) const
{
    for (auto* taskbar_wnd : m_tBarDlg_all)
    {
        if (taskbar_wnd != nullptr && taskbar_wnd->GetSafeHwnd() == hwnd)
            return taskbar_wnd;
    }
    return nullptr;
}

void CTrafficMonitorDlg::ForEachTaskbarWnd(const std::function<void(CTaskBarDlg*)>& callback)
{
    for (auto* taskbar_wnd : m_tBarDlg_all)
    {
        if (taskbar_wnd != nullptr && ::IsWindow(taskbar_wnd->GetSafeHwnd()))
            callback(taskbar_wnd);
    }
    if (!m_tBarDlg_all.empty())
        m_tBarDlg = m_tBarDlg_all.front();
    else
...
void CTrafficMonitorDlg::ApplySettings(COptionsDlg& optionsDlg)
{
    ...
    if (IsTaskbarWndValid())
    {
        ForEachTaskbarWnd([](CTaskBarDlg* taskbar_wnd) { taskbar_wnd->ApplySettings(); });
...
void CTrafficMonitorDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    if (nIDEvent == MAIN_TIMER)
    {
        ...
        if (!theApp.m_cfg_data.m_hide_main_window || theApp.m_cfg_data.m_show_task_bar_wnd)
        {
            ...
            {
                bool has_insert_failed{};
                int error_code{};
                ForEachTaskbarWnd([&](CTaskBarDlg* taskbar_wnd)
                    {
                        if (taskbar_wnd->GetCannotInsertToTaskBar())
                        {
                            has_insert_failed = true;
                            if (error_code == 0)
                                error_code = taskbar_wnd->GetErrorCode();
                        }
                    });

                if (has_insert_failed && m_insert_to_taskbar_cnt < MAX_INSERT_TO_TASKBAR_CNT)
                {
                    CloseTaskBarWnd();
                    OpenTaskBarWnd();
                    m_insert_to_taskbar_cnt++;

                    has_insert_failed = false;
                    error_code = 0;
                    ForEachTaskbarWnd([&](CTaskBarDlg* taskbar_wnd)
                        {
                            if (taskbar_wnd->GetCannotInsertToTaskBar())
                    ...
                    if (has_insert_failed && m_insert_to_taskbar_cnt >= WARN_INSERT_TO_TASKBAR_CNT)
                    {
                        //写入错误日志
                        ...
                        info.Replace(_T("<%error_code%>"), CCommon::IntToString(error_code));
                        CCommon::WriteLog(info, theApp.m_log_path.c_str());
                        if (m_cannot_insert_to_task_bar_warning)      //确保提示信息只弹出一次
                        {
                            //弹出错误信息
                            m_cannot_insert_to_task_bar_warning = false;
                            MessageBox(CCommon::LoadText(IDS_CONNOT_INSERT_TO_TASKBAR, CCommon::IntToString(error_code)), NULL, MB_ICONWARNING);
                        }
                    }
                }
        ...
        {
            COLORREF sampled_color = CLR_INVALID;
            ForEachTaskbarWnd([&](CTaskBarDlg* taskbar_wnd)
                {
                    if (sampled_color != CLR_INVALID)
                        return;
                    ...
                    int virtual_right = virtual_left + ::GetSystemMetrics(SM_CXVIRTUALSCREEN) - 1;
                    int virtual_bottom = virtual_top + ::GetSystemMetrics(SM_CYVIRTUALSCREEN) - 1;
                    if (pointx < virtual_left) pointx = virtual_left;
                    if (pointx > virtual_right) pointx = virtual_right;
                    if (pointy < virtual_top) pointy = virtual_top;
                    if (pointy > virtual_bottom) pointy = virtual_bottom;

                    COLORREF color = ::GetPixel(m_desktop_dc, pointx, pointy);        //取任务栏窗口边缘1像素处的颜色作为背景色
                    if (color != CLR_INVALID)
                        sampled_color = color;
                });

            if (sampled_color != CLR_INVALID
                && !CCommon::IsColorSimilar(sampled_color, theApp.m_taskbar_data.back_color)
                && sampled_color != 0)
            {
                bool is_taskbar_transparent{ theApp.m_taskbar_data.IsTaskbarTransparent()};
                theApp.m_taskbar_data.back_color = sampled_color;
    ...
    if (nIDEvent == TASKBAR_TIMER)
    {
        ...
        if (IsTaskbarWndValid())
        {
            ForEachTaskbarWnd([this](CTaskBarDlg* taskbar_wnd)
                {
                    ...
                    {
                        CTaskBarDlg::CheckWindowMonitorDPIAndHandle(*taskbar_wnd, [p_taskbar_wnd = taskbar_wnd](UINT new_dpi_x, UINT)
                            {
                                //考虑到任务栏窗口可能和主窗口不在同一个屏幕上，dpi可能不同
                                //设置DPI并刷新窗口
                                p_taskbar_wnd->SetDPI(new_dpi_x);
                                p_taskbar_wnd->SetTextFont();
                                p_taskbar_wnd->CalculateWindowSize();
                            });
                    }
...
void CTrafficMonitorDlg::OnNetworkInfo()
{
    ...
    ForEachTaskbarWnd([](CTaskBarDlg* taskbar_wnd)
        {
            taskbar_wnd->m_tool_tips.SetWindowPos(&wndTopMost, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
        });  //重新设置任务栏窗口的提示信息置顶
}
...
{
    CWnd* pParent = this;
    CTaskBarDlg* target_taskbar = GetTaskbarWndByHwnd(m_taskbar_menu_source_hwnd);
    if (target_taskbar != nullptr)
        pParent = target_taskbar;
...
{
    m_taskbar_menu_source_hwnd = reinterpret_cast<HWND>(wParam);
    //设置“选择连接”子菜单项中各单选项的选择状态
...
afx_msg LRESULT CTrafficMonitorDlg::OnMonitorInfoUpdated(WPARAM wParam, LPARAM lParam)
{
    ...
    //更新任务栏窗口鼠标提示
    ForEachTaskbarWnd([](CTaskBarDlg* taskbar_wnd) { taskbar_wnd->UpdateToolTips(); });
    return 0;
}
...
void CTrafficMonitorDlg::OnPluginOptionsTaksbar()
{
    CTaskBarDlg* target_taskbar = GetTaskbarWndByHwnd(m_taskbar_menu_source_hwnd);
    if (target_taskbar == nullptr)
        target_taskbar = m_tBarDlg;

    //判断任务栏窗口中点击的项目是否是插件项目
    if (target_taskbar != nullptr && target_taskbar->GetClickedItem().IsPlugin())
    {
        //找到对应的插件
        ITMPlugin* plugin = theApp.m_plugins.GetPluginByItem(target_taskbar->GetClickedItem().PluginItem());
        if (plugin != nullptr)
        {
            //显示插件的选项设置
            auto rtn = plugin->ShowOptionsDialog(target_taskbar->GetSafeHwnd());
